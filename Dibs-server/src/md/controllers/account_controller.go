...
// AccountControllerImpl note
type AccountControllerImpl struct {
	scheme   string
	database utils.DatabaseAccessor
}

// NewAccountController note
func NewAccountController(database utils.DatabaseAccessor, isDevelopment bool) *AccountControllerImpl {
	scheme := "http"
	if !isDevelopment {
		scheme += "s"
	}
...
// Register note
func (ac *AccountControllerImpl) Register(r *gin.Engine) {
	// auth
...
func (ac *AccountControllerImpl) login(c *gin.Context) {
	c.JSON(200, gin.H{
		"message": "pong",
	})
}
